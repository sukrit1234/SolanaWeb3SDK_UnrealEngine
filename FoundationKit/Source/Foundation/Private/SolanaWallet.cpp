...
bool USolanaWallet::SetSaveSlotName(FString NewSaveSlotName)
{
	if (IsWalletLocked())
		return false;
	if (SaveSlotName == NewSaveSlotName) 
		return false;
	...
	if (bDoesWalletSaveExits && !bDeleted)
	{
		...
		for (const auto& PublicKey : PublicKeys)
		{
			...
			if (IsValid(Account))
			{
				Account->BindAsyncTaskManager(AsyncTaskMgr);
...
bool USolanaWallet::SaveWallet()
{
	...
	for (auto It = Accounts.CreateConstIterator(); It; ++It)
	{
		UWalletAccount* Account = It.Value();
		if (IsValid(Account))
...
bool USolanaWallet::UnlockWallet(FString Password, FText& FailReason)
{
	...
	for (int32 i = 0; i < CurrentSaveData->Accounts.Num(); ++i)
	{
		...
		Account->AccountData.GenerateData();
...
void USolanaWallet::LockWallet(bool bSaveWallet)
{
...
#else
		CurrentSaveData->MarkPendingKill();
#endif
	...
	for (auto It = Accounts.CreateConstIterator(); It; ++It)
	{
		UWalletAccount* Account = It.Value();
		if(IsValid(Account))
			Account->AccountData = FAccount::FromPublicKey(It.Key());
	}
	/*for (auto& [PublicKey, Account] : Accounts)
	{
...
UWalletAccount* USolanaWallet::GetAccountFromGenIndex(int32 GenIndex) const
{
	...
	{
		UWalletAccount* Account = It.Value();
		if (IsValid(Account) && Account->AccountData.GenIndex == GenIndex)
			return Account;
	}
	/*for (auto& [PublicKey, Account] : Accounts)
	{
...
void USolanaWallet::UpdateAccounts()
{
	...
	FSolanaRequestMultipleAccountAsyncTask* NewTask = new FSolanaRequestMultipleAccountAsyncTask(FRequestManager::GetNextMessageID(), GetPublicKeys());
	NewTask->Callback.BindLambda([this](UINT Id, const TArray<FAccountInfoJson>& AccountInfos)
		{
			TArray<UWalletAccount*> CurrentAccounts = GetAccounts();
			for (int32 Index = 0; Index < AccountInfos.Num(); ++Index)
			{
				const FAccountInfoJson& AccountInfoJson = AccountInfos[Index];
