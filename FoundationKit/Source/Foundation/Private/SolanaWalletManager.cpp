...
void USolanaWalletManager::Deinitialize()
{
	Super::Deinitialize();
	if (AsynTaskManager.IsValid())
		AsynTaskManager->Shutdown();
}
...
{
	USolanaWallet* NewWallet = NewObject<USolanaWallet>(this);
	if (IsValid(NewWallet))
		NewWallet->BindAsyncTaskManager(GetAsynTaskManager());
	return NewWallet;
}
...
USolanaWallet* USolanaWalletManager::GetOrCreateWallet(const FString& SlotName)
{
	...
	if (IsValid(Wallet))
	{
		Wallet->BindAsyncTaskManager(GetAsynTaskManager());
...
const FApiAsyncTaskSubSystemPtr& USolanaWalletManager::GetAsynTaskManager()
{
	if (!AsynTaskManager.IsValid())
	{
		AsynTaskManager = FApiAsyncTaskSubSystemPtr(new FApiAsyncTaskSubSystem("Solana"));
		AsynTaskManager->Init();
	}
	return AsynTaskManager;
}