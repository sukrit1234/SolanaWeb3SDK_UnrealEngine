...
{
	FSolanaRequestTokenAccountAsyncTask* NewTask = new FSolanaRequestTokenAccountAsyncTask(FRequestManager::GetNextMessageID(), AccountData.Pubkey, AccountData.Mint);
	NewTask->Callback.BindLambda([this](UINT Id, const FTokenBalanceDataJson& Data)
		{
			FTokenInfoJson info = Data.account.data.parsed.info;
