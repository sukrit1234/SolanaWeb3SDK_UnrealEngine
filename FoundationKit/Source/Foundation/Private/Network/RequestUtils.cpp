...
{
	TSharedPtr<FJsonObject> ResultPtr = data.GetObjectField("result");
	return ResultPtr.IsValid() ? ParseAccountInfoResult(*ResultPtr) : FAccountInfoJson::Empty;
}
FAccountInfoJson FRequestUtils::ParseAccountInfoResult(const FJsonObject& result)
{
...
#else
		FJsonObjectConverter::JsonArrayToUStruct(result->GetArrayField("value"), &jsonData, 0, 0);
#endif
	}
	return jsonData;
}*/
